...
from beatreel.pipeline import _cut_length_for, _plan_cuts_greedy
...
def test_plan_cuts_greedy_respects_target_duration():
    beats = _beats_at_120_bpm()
    highlights = _mk_highlights(["a.mp4", "b.mp4", "c.mp4"], per_clip=4)
    plans = _plan_cuts_greedy(highlights, beats, target_duration=10.0, intensity="balanced")
    total = sum(p.duration for p in plans)
    assert 0 < total <= 10.0 + 0.1
    assert len(plans) > 0


def test_plan_cuts_greedy_empty_highlights():
    beats = BeatGrid(tempo=120.0, beat_times=np.array([]), downbeat_times=np.array([]), duration=0.0)
    assert _plan_cuts_greedy([], beats, 60.0, "balanced") == []


def test_plan_cuts_greedy_seed_determinism_same_seed():
    beats = _beats_at_120_bpm()
    highlights = _mk_highlights([f"clip_{i}.mp4" for i in range(5)], per_clip=3)
    a = _plan_cuts_greedy(highlights, beats, target_duration=20.0, intensity="balanced", seed=42)
    b = _plan_cuts_greedy(highlights, beats, target_duration=20.0, intensity="balanced", seed=42)
    assert [(p.clip_path, p.start, p.duration) for p in a] == [
        (p.clip_path, p.start, p.duration) for p in b
    ]


def test_plan_cuts_greedy_different_seeds_differ_sometimes():
    ...
    for seed in range(10):
        plans = _plan_cuts_greedy(highlights, beats, target_duration=15.0, intensity="balanced", seed=seed)
...
def test_plan_cuts_greedy_does_not_overlap_same_source_clip():
    beats = _beats_at_120_bpm()
    ...
    plans = _plan_cuts_greedy(highlights, beats, target_duration=20.0, intensity="balanced")
